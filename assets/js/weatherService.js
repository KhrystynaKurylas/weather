...
function getWeatherData(lang, fnOK, fnError) {
    ...
    function locSuccess(position) {
        ...
        if(cache && cache.timestamp && cache.timestamp > currDate.getTime() - 30*60*1000){
            ...
            $.getJSON(
               'http://api.openweathermap.org/data/2.5/forecast?lat='+position.coords.latitude+
		'&lon='+position.coords.longitude+'&cnt=16&units=metric&lang=en&callback=?&appid=9649c92439ade00fa8117dec4613064b'
                function (response) {
                    // Store the cache
